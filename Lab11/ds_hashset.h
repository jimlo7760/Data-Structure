...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    if (m_table[index].size() > 0){
        ...
        while (itr != (this -> m_table)[index].end()){
            if ((*itr) == key) {
                iterator retItr(this, index, itr);
                return std::make_pair(retItr, false);
            }
            itr++;
        }
    }
    (this -> m_table)[index].push_back(key);
    itr = (this -> m_table)[index].end();
    itr--;
    iterator retItr(this, index, itr);
    this -> m_size++;
    return std::make_pair(retItr, true);
  ...
  iterator begin() {
    ...
    for (unsigned int i = 0; i < (this->m_table).size(); i++){
        if ((this->m_table)[i].size() > 0){
            hash_list_itr itr = (this->m_table)[i].begin();
            iterator retItr(this, i, itr);
            return retItr;
        }
    }
    return this->end();
