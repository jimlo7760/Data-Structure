...
typename dslist<T>::iterator dslist<T>::insert(iterator itr, const T& v) {
    ...
    if (this->size_ == 0){
        ...
        // insert into the last element
        // office hour:
    }else{
        temp->next_ = itr.ptr_;
        temp->prev_ = (itr.ptr_)->prev_;
        ((itr.ptr_)->prev_)->next_ = temp;
        (itr.ptr_)->prev_ = temp;
    }
