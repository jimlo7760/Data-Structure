...
void print_grid(const std::vector<std::vector<GRID_STATUS> > &blocked_grid,
                unsigned int start_x, unsigned int start_y, std::vector<int> xPath, std::vector<int> yPath) {
    ...
    std::cout << "After the path" << std::endl;
    for (unsigned int y = 0; y < blocked_grid[0].size(); ++y) {
        for (unsigned int x = 0; x < blocked_grid.size(); ++x) {
            bool printed = false;
...
bool
calPath(std::vector<std::vector<GRID_STATUS> > grid, int &x, int &y, std::vector<int> &xPath, std::vector<int> &yPath) {
    if (grid[x][y] == GRID_BLOCKED) {
        return false;
    }
    xPath.push_back(x);
    yPath.push_back(y);
    ...
    if (x == 0) {
        int reducedY = y - 1;
        retVal = calPath(grid, x, reducedY, xPath, yPath);
        if (retVal) {
            return true;
        }
        xPath.pop_back();
    ...
    if (y == 0) {
        int reducedX = x - 1;
        retVal = calPath(grid, reducedX, y, xPath, yPath);
        if (retVal) {
            return true;
        }
        xPath.pop_back();
...
int main(int argc, char *argv[]) {
    ...
    print_grid(blocked_grid, start_x, start_y, xPath, yPath);

    std::cout << calPath(blocked_grid, start_x, start_y, xPath, yPath);
    print_grid(blocked_grid, start_x, start_y, xPath, yPath);
