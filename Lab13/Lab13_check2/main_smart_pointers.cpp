...
void deleteHelper(std::set<const Balloon *> &idenBall, const Balloon *curBall) {
    if (idenBall.find(curBall) != idenBall.end()){
        return;
    }
    idenBall.insert(curBall);
    int i = 0;
    for (int i = 0; i < curBall->numRopes(); i++){
        deleteHelper(idenBall, curBall ->getRope(i));
    }
}

void deleteAll(Balloon *root) {
    std::set<const Balloon *> idenBall;
    const Balloon* constRoot = root;
    deleteHelper(idenBall, constRoot);
    std::set<const Balloon*>::iterator itr = idenBall.begin();
    while (itr != idenBall.end()){
        std::set<const Balloon*>::iterator next = ++itr;
        itr--;
        delete *itr;
        itr = next;
    }
